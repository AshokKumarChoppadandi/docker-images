...
if [[ $1 = "stop" ]]; then
  echo "Stopping the Hadoop & Spark Services...!!!"
...
if [[ $1 = "deploy" ]]; then
  ...
  docker run  -d --net hadoop_network --ip 172.18.1.11 --hostname mysql-metastore -p 3306:3306 --add-host nodemaster:172.18.1.1 --add-host node2:172.18.1.2 --add-host node3:172.18.1.3 --add-host node4:172.18.1.4 --name mysql-metastore -e MYSQL_ROOT_PASSWORD=123456 ashokkumarchoppadandi/mysql:latest

  echo "Deploying Hadoop & Spark Master Node"
  docker run -d --net hadoop_network --ip 172.18.1.1 --hostname nodemaster --add-host mysql-metastore:172.18.1.11 --add-host node2:172.18.1.2 --add-host node3:172.18.1.3 --add-host node4:172.18.1.4 -p 50070:50070 -p 8032:8032 -p 8020:8020 -p 50030:50030 -p 19888:19888 -p 8088:8088 -p 8080:8080 -p 4040:4040 --link mysql-metastore --name nodemaster -it ashokkumarchoppadandi/hadoopspark:latest bash

  echo "Deploying Hadoop & Spark Worker Nodes"
  docker run -d --net hadoop_network --ip 172.18.1.2 --hostname node2 --add-host mysql-metastore:172.18.1.11 --add-host nodemaster:172.18.1.1 --add-host node3:172.18.1.3 --add-host node4:172.18.1.4 --link nodemaster --link mysql-metastore --name node2 -it ashokkumarchoppadandi/hadoopspark:latest bash
  docker run -d --net hadoop_network --ip 172.18.1.3 --hostname node3 --add-host mysql-metastore:172.18.1.11 --add-host nodemaster:172.18.1.1 --add-host node2:172.18.1.2 --add-host node4:172.18.1.4 --link nodemaster --link mysql-metastore --name node3 -it ashokkumarchoppadandi/hadoopspark:latest bash
  docker run -d --net hadoop_network --ip 172.18.1.4 --hostname node4 --add-host mysql-metastore:172.18.1.11 --add-host nodemaster:172.18.1.1 --add-host node2:172.18.1.2 --add-host node3:172.18.1.3 --link nodemaster --link mysql-metastore --name node4 -it ashokkumarchoppadandi/hadoopspark:latest bash
